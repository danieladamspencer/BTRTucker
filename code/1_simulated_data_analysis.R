...
parLapplyLB(cl, rev(rank_values), function(ranks) {
  ...
  saveRDS(bayes_result,
          paste0(
            "results/1_simulated_data_results_rank",
...
parSapply(cl, seq(4), function(rank) {
  ...
  saveRDS(bayes_result,
          paste0(
            "results/1_simulated_data_btr_cp_results_rank",
            rank,
            ".rds"
          ))
  return(NULL)
...
rank_values <- expand.grid(r1 = 1:4, r2 = 1:4)
rank_values <- split(rank_values,row(rank_values))
lapply(rank_values, function(ranks) {
  # >> Perform the analyses using the bayestensorreg package ----
  ftr_result <-
    FTRTucker(
      y = sim_data$y,
  ...
  saveRDS(ftr_result,
          paste0(
            "results/1_simulated_data_ftrtucker_results_rank",
            paste(as.numeric(ranks), collapse = ""),
            ".rds"
          ))
  return(NULL)
...
sapply(seq(4), function(rank) {
  ...
  saveRDS(ftr_result,
          paste0(
            "results/1_simulated_data_ftr_cp_results_rank",
            rank,
            ".rds"
          ))
  return(NULL)
